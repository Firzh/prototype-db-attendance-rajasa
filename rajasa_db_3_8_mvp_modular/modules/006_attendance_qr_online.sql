...
ALTER TABLE `log_scan_qr`
  ...
  ADD COLUMN `scan_source` ENUM('legacy_esp32','web_hp','web_tablet','web_desktop','manual_web','system') NOT NULL DEFAULT 'web_hp' COMMENT 'Sumber kejadian scan. Untuk alur baru umumnya web_hp.' AFTER `scan_method`,
  ADD COLUMN `context_type` ENUM('rombel','ruangan','jadwal','legacy_esp32') NOT NULL DEFAULT 'web_hp' COMMENT 'Konteks scan saat kejadian. Untuk opsi A memakai rombel.' AFTER `scan_type`,
